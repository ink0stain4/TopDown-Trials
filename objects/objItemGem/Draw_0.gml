...
if state != ItemState.TRASHING
{
	...
	var _heightOffset = (sprite_get_height(sprite_index) / 3) // i change depending on sprite
	var _shadow_scale = max(0.12, 0.4 - (z / 100)); 
...
if (hovered)
{
	...
	draw_sprite_ext(
		...
		_cx,
		_cy,
		_tw / sprite_get_width(sprTooltip),
		_th / (sprite_get_height(sprTooltip) + 4),
		0,
		c_white,
		1)
...
if global.debug
{
	...
	draw_set_colour(c_white)
	draw_text(x, (y-14) - z, floor(alarm[1]/60))
}